...
def multi_isotropic_psd_2d(x, spacing, nbins=50, hann=True, return_all=False, log_bins=False):
    """
    x : 3D array (nt, ny, nx)
    spacing : grid spacing in meters (assumed equal in x & y)
    nbins : number of radial wavenumber bins
    ...
    for t in range(nt):
        k_center, psd_iso = isotropic_psd_2d(x[t, :, :], spacing, nbins, hann=hann, log_bins=log_bins)
...
def samples_psd_stats(
    ...
    return k, psd_mean, psd_1sigma_lower, psd_1sigma_upper, psd_2sigma_lower, psd_2sigma_upper
...
def calculate_theoretical_psd_matern(f, sigma, l, nu, n, spacing, tau=0.0):
    ...
    numerator = (sigma**2) * (2**n) * (np.pi**(n/2)) * gamma(nu + n/2) * ((2 * nu)**nu)
