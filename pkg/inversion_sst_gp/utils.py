...
def geo_to_car(lon, lat, lonc, latc):
    ...
    for i in range(Nx):
        for j in range(Ny):
            X[j, i] = (
                np.sign(lon[i] - lonc)
                * Geodesic.WGS84.Inverse(lat[j], lon[i], lat[j], lonc)["s12"]
            )
            Y[j, i] = (
                np.sign(lat[j] - latc)
                * Geodesic.WGS84.Inverse(lat[j], lon[i], latc, lon[i])["s12"]
            )
    return X, Y
...
def finite_difference_2d(s1_2d, s2_2d, x_2d):
    ...
    dxds2 = np.stack(
        [finite_difference_1d(s2_2d[:, i], x_2d[:, i]) for i in range(N1)]
    ).T
    return dxds1, dxds2
...
def calculate_exponential_line(k_0, f_0, k_1, slope):
    # calculate the exponential line between two points with a given slope
    f_1 = f_0 * (k_1 / k_0) ** slope
    k_points = [k_0, k_1]
    f_points = [f_0, f_1]
...
def calculate_geostrophic_velocity_fields(X, Y, LAT, eta):
    ...
    detadx, detady = finite_difference_2d(X, Y, eta)
