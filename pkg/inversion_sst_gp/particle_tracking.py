...
def simulate_particle_flow_field(X, Y, u, v, lon0, lat0, lonc, latc, tstep, N):
    ...
    for i in range(1, N):
        ...
        geo_dict = Geodesic.WGS84.Direct(
            latc, lonc, np.rad2deg(np.arctan2(xp, yp)), np.sqrt(xp**2 + yp**2)
        )
...
def simulate_particle_multi_flow_fields(
    ...
    for i in range(n_sample):
        lons[i, :], lats[i, :] = simulate_particle_flow_field(
            X, Y, us[i, :, :], vs[i, :, :], lon0, lat0, lonc, latc, tstep, Nstep
        )
    return lons, lats
