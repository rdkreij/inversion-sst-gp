...
def run_gp_regression_and_metrics(dTds1o, dTds2o, dTdto, params_path, param_key, param_val, param_type, X, Y, time_step, u, v):
    print(f"Extracting parameters from {params_path} for {param_key}={param_val} ({param_type})")
    params = utils.extract_params(params_path, param_key, param_val, type=param_type)
    print("Calculating GP regression predictions")
    muustar, muvstar, muSstar, stdustar, stdvstar, stdSstar, Kxstar_vel = gp_regression.calculate_prediction_gpregression(
...
def run_global_optimal_solution(dTds1o, dTds2o, dTdto, params_path, param_key, param_val, param_type, u, v):
    print(f"Extracting GOS parameters from {params_path} for {param_key}={param_val} ({param_type})")
    params = utils.extract_params(params_path, param_key, param_val, type=param_type)
    n_gos = int(params["n"])
...
def plot_and_save_predictions(
    LON, LAT, To, dTds1o, dTds2o, dTdto,
    muSstar, Kxstar_vel, stdSstar, muustar, muvstar,
    lonlims, latlims,
    u=None, v=None, S=None,
    ugos=None, vgos=None, Sgos=None,
    filename=None
):
    print(f"Plotting and saving predictions to {filename}")
...
def experiment_fully_observed():
    ...
    data = load_and_prepare_dataset("1_preproc_data/proc_data/suntans_1h.nc",
                                    "time", np.datetime64("2014-02-19T18:00:00"))
    muustar, muvstar, muSstar, stdustar, stdvstar, stdSstar, Kxstar_vel, metrics_gp = run_gp_regression_and_metrics(
        data["dTds1o"], data["dTds2o"], data["dTdto"],
        "2_covariance_parameter_estimation/outputs/noise_gp_obs_t.csv",
        "noise_sd", 0, "gp",
        data["X"], data["Y"], data["time_step"],
        data["u"], data["v"],
    )
    ...
    plot_and_save_predictions(
        data["LON"], data["LAT"], data["To"],
        data["dTds1o"], data["dTds2o"], data["dTdto"],
        muSstar, Kxstar_vel, stdSstar, muustar, muvstar,
        lonlims, latlims,
        u=data["u"], v=data["v"], S=data["S"],
        ugos=ugos, vgos=vgos, Sgos=Sgos,
        filename="3_observing_system_simulation_experiment/figures/osse_instance_fully_observed.png",
    )
    transect = store_transect_data(data["lon"], data["To"], data["v"], muvstar, stdvstar)
    return transect
...
def experiment_dense_cloud(coverage_dense=0.3):
    ...
    muustar, muvstar, muSstar, stdustar, stdvstar, stdSstar, Kxstar_vel, metrics_gp = run_gp_regression_and_metrics(
        ...
        "coverage_dense", coverage_dense, "gp",
        data["X"], data["Y"], data["time_step"],
        data["u"], data["v"],
    )
    plot_and_save_predictions(
...
def experiment_sparse_cloud(coverage_sparse=0.3):
    ...
    muustar, muvstar, muSstar, stdustar, stdvstar, stdSstar, Kxstar_vel, metrics_gp = run_gp_regression_and_metrics(
        ...
        "coverage_sparse", coverage_sparse, "gp",
        data["X"], data["Y"], data["time_step"],
        data["u"], data["v"],
    )
    plot_and_save_predictions(
...
def plot_and_save_transects(transect_fully_observed, transect_measurement_error, transect_dense_cloud, transect_sparse_cloud):
    print("Plotting and saving combined transects overview")
...
def main():
    ...
    transect_sparse_cloud = experiment_sparse_cloud(coverage_sparse=0.3)
